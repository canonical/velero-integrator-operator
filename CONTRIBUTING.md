...
* **`tox -e integration`** — Tests data propagation through relation databags. Only requires velero-operator from Charmhub (no S3 storage needed).
* **`tox -e integration-e2e`** — End-to-end tests that verify velero-operator actually creates Velero Schedule CRs in Kubernetes. Requires S3-compatible storage.

**Important things to know before running the e2e integration tests:**

* These tests require AWS-style S3 credentials.
* If you're running in a CI environment (`CI=true`), a local RadosGW (via `microceph`) will be created automatically.
* When testing locally, you **must** provide your own credentials or reuse those from `microceph`.

#### 1. Setup RadosGW

The e2e integration tests can install microceph with RadosGW, and then run the tests. To do so you'll need to run the integration tests with `CI=true`.
...
```bash
CI=true tox -vve integration-e2e
```

#### 2. Reuse Local RadosGW

You can also run the e2e integration tests and point them to an existing local instance of RadosGW. For example, set the following environment variables:
...
Then you can run the e2e integration tests with:

```bash
tox -vve integration-e2e
```
...
```bash
export CHARM_PATH=/path/to/velero-integrator_ubuntu-24.04-amd64.charm
export TEST_APP_CHARM_PATH=/path/to/test-app_ubuntu-24.04-amd64.charm
