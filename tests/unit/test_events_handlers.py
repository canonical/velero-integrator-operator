...
class TestVeleroBackupEventsDirectCoverage:
    """Direct tests for VeleroBackupEvents to ensure full coverage."""

    def test_publish_to_relation_with_none_relation(self):
        """Test publish_to_relation returns early when relation is None."""
        mock_charm = MagicMock()
        mock_charm.unit.is_leader.return_value = True
        mock_context = MagicMock()

        with patch.object(VeleroBackupEvents, "__init__", lambda self, c, ctx: None):
            handler = VeleroBackupEvents.__new__(VeleroBackupEvents)
            handler.charm = mock_charm
            handler.context = mock_context

            # Call with relation=None - should return early
            handler.publish_to_relation(None)

            # Should return early without accessing context.config
            mock_context.config.assert_not_called()

    def test_publish_to_relation_with_invalid_config(self):
        """Test publish_to_relation returns early when config is None."""
        mock_charm = MagicMock()
        mock_charm.unit.is_leader.return_value = True
        ...
        with (
            patch.object(VeleroBackupEvents, "__init__", lambda self, c, ctx: None),
            patch.object(VeleroBackupEvents, "logger", new_callable=PropertyMock) as logger_prop,
        ):
            logger_prop.return_value = mock_logger

            handler = VeleroBackupEvents.__new__(VeleroBackupEvents)
            handler.charm = mock_charm
            handler.context = mock_context

            # Make config return None (invalid config)
            type(mock_context).config = PropertyMock(return_value=None)

            handler.publish_to_relation(mock_relation)

            # Should log warning and return
            mock_logger.warning.assert_called_once()
            assert "Invalid config" in mock_logger.warning.call_args[0][0]

    def test_publish_to_all_relations_non_leader(self):
        """Test publish_to_all_relations returns early for non-leader."""
        mock_charm = MagicMock()
        ...
        with patch.object(VeleroBackupEvents, "__init__", lambda self, c, ctx: None):
            ...
            handler.publish_to_all_relations()

            # Should not access velero_relations since non-leader returns early
            mock_context.velero_relations.assert_not_called()
